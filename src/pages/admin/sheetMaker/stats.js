...
const Stats = () => {
    ...
    /*
            [
                {
                    title: "Last Run Execution Time",
                    stats: `${mostRecentRun['total_execution_time'].seconds}.${mostRecentRun['total_execution_time'].milliseconds} seconds`,
                    description: "Time taken to run the most recent run"
                },
                {
                    title: "Current File Count",
                    stats: mostRecentRun['opps_in_run'],
                    description: "Opps processed in the most recent run"
                },
                {
                    title: "Time Since Last Folder",
                    stats: daysSinceLastPO ? daysSinceLastPO : "N/A",
                    description: lastRunToGenerateFolder ? ` A Folder was last generated on ${format(serverDateFix(new Date(lastRunToGenerateFolder['start_date'])), "Pp")}` : "No POs generated"
                }
            ],
            [
                {
                    title: "Best Execution Time",
                    stats: minDuration(runs.map(run => run['total_execution_time'])) + " seconds",
                ...
                },
                {
                    ...
                    stats: formatDuration(intervalToDuration({
                        start: 0,
                        end: averageExecutionTime
                    })).replace("minutes", "min").replace("seconds", "sec"),
                    description: "Average time per run"
                },
                {
                    title: "Worst Execution Time",
                    stats: formatDuration(secondsToDuration(maxDuration(runs.map(run => run['total_execution_time'])))).replace("minutes", "min").replace("seconds", "sec"),
                    description: "Best time per run"
                },
            ],
            [
                {
                    title: "Current Opp Total",
                    stats: mostRecentRun['opps_in_run'],
                    description: "Total opps in the most recent run"
                },
                {
                    title: "Current Opps Attempted",
                    stats: mostRecentRun['opps_attempted'],
                    description: "Opps processed in the most recent run"
                },
                {
                    title: "Current fail count",
                    stats: mostRecentRun['opps_attempted'] - mostRecentRun['opps_processed'],
                    description: "Opps skipped due to being in the paused state"
                }
            ],
            [
                {
                    title: "Best Processing Time",
                    stats: formatDuration(secondsToDuration(minDuration(runs.map(run => run['total_time_processing_opps']))))
                        .replace("minutes", "min").replace("seconds", "sec"),
                    description: "Time Processing Opps"
                },
                {
                    title: "Average Processing Time",
                    stats: formatDuration(secondsToDuration(averageDuration(runs.map(run => run['total_time_processing_opps']))))
                        .replace("minutes", "min").replace("seconds", "sec"),
                    description: "Time Spent processing Opps"
                },
                {
                    title: "Worst Processing Time",
                    stats: formatDuration(secondsToDuration(maxDuration(runs.map(run => run['total_time_processing_opps']))))
                        .replace("minutes", "min").replace("seconds", "sec"),
                    description: "Time Spent processing Opps"
                },
            ],
            ...
            [
                ...
                {
                    title: "Total Folders Generated",
                    stats: runs.reduce((acc, run) => acc + run['folders_generated'] + 373 , 0),
                    description: "Total Folders generated"
                },
                {
                    title: "Time Since Last PO",
