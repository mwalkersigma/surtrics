...
async function getHandler(){
    return await db.query(`
        WITH increments AS (SELECT COUNT(*)                              AS total_increments,
                                   DATE_TRUNC('month', transaction_date) as month_of_transaction
                            FROM surtrics.surplus_metrics_data
             ...
             relisting AS (SELECT COUNT(*)                              as transactions,
             ...
             new_listing AS (SELECT COUNT(*)                              as transactions,
                                    date_trunc('month', transaction_date) as date_of_transaction
                             FROM surtrics.surplus_metrics_data
                             WHERE transaction_type = 'Add'
                               AND (
                             ...
                             GROUP BY date_of_transaction),
             big_commerce AS (SELECT SUM(visits)                        as visits,
                              ...
                              GROUP BY month_bc),
             ebay AS (SELECT SUM(impressions)                   as impressions,
                             SUM(page_views)                    as page_views,
                             DATE_TRUNC('month', date_for_week) as month_ebay
                      FROM surtrics.surplus_ebay_data
                      GROUP BY month_ebay),
             quickbooks AS (SELECT DATE_TRUNC('month', po_date) as month_quickbooks,
                                   count(*)                     as po_count,
                                   AVG(purchase_total)          as po_avg,
                                   SUM(purchase_total)          as po_total
                            FROM surtrics.surplus_quickbooks_data
                            GROUP BY month_quickbooks)
        SELECT TO_CHAR(month_of_transaction, 'YYYY-MM-DD') as month,
               impressions,
               page_views,
               visits,
